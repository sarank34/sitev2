...
/* V5 premium boot name animation + light mode button */
const bootName = document.getElementById("bootName");
const mobileThemeToggle = document.getElementById("mobileThemeToggle");

function renderBootName() {
  if (!bootName) return;
  const text = "SARAN K.";
  bootName.innerHTML = "";
  let delay = 0;
  [...text].forEach((ch) => {
    if (ch === "\\n") {
      bootName.appendChild(document.createElement("br"));
      delay += 90;
      return;
    }
...
function syncLightButton() {
  if (!mobileThemeToggle) return;
  mobileThemeToggle.textContent = document.documentElement.dataset.theme === "light" ? "Dark Mode" : "Light Mode";
}
...
syncLightButton();
document.getElementById("themeToggle")?.addEventListener("change", syncLightButton);


/* V7 cleaner boot timing + landing reveal */
...
document.querySelectorAll(".landing-btn,.landing-link").forEach((el)=>{
  el.addEventListener("pointerdown",()=>{ if("vibrate" in navigator) navigator.vibrate(8); },{passive:true});
});
