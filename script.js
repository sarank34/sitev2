...
/* V11 stable matrix boot + polished controls */
(() => {
  const boot = document.getElementById("boot");
  ...
  const closeBoot = () => {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        if (location.hash && location.hash !== "#landing") history.replaceState(null, "", location.pathname + location.search);
        window.scrollTo({ top: 0, left: 0, behavior: "instant" });
      } catch (error) {}
    }, 720);
  };

  window.addEventListener("load", () => setTimeout(closeBoot, 1900), { once: true });
  ...
  function filterProjects() {
    ...
    cards.forEach((card) => {
      ...
      const visible = (filter === "all" || card.dataset.category === filter) && (!query || text.includes(query));
      card.hidden = !visible;
    });
  }
  ...
  function setTheme(mode) {
    ...
    try { localStorage.setItem("saran-grid-theme", mode); } catch (error) {}
    if (themeToggle) themeToggle.checked = mode === "light";
    if (mobileTheme) mobileTheme.textContent = mode === "light" ? "Dark Mode" : "Light Mode";
  }

  setTheme(localStorage.getItem("saran-grid-theme") || root.dataset.theme || "dark");

  themeToggle?.addEventListener("change", () => setTheme(themeToggle.checked ? "light" : "dark"));
  mobileTheme?.addEventListener("click", () => {
    setTheme(root.dataset.theme === "light" ? "dark" : "light");
    if ("vibrate" in navigator) navigator.vibrate([8, 12, 8]);
  });
